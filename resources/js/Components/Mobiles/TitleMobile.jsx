...
export default function TitleMobile({
  ...
  return (
    ...
    {
      (!showSearch && !showDate) && 
      <div className={`sm:hidden fixed mt-12 w-full bg-white border px-3 py-1 flex justify-between shadow-lg ${zIndex}`}>
        {/* search */}                        
    ...
    {
      showDate && 
      <div className={`sm:hidden fixed mt-12 w-full bg-white border px-3 py-1 justify-between shadow-lg ${zIndex} p-1`}>
        <div className='flex p-1'>
          <div className='w-1/12 my-auto'>
            <button onClick={() => setShowDate(false)} className='my-auto p-2'><IoArrowBackOutline /></button>
          </div>
    ...
    
    {
      ...
      <div className={`sm:hidden fixed mt-12 w-full bg-white border px-3 py-1 justify-between shadow-lg ${zIndex} p-1`}>
        <div className='flex p-1'>
          <div className='w-1/12 my-auto'>
            <button onClick={() => setShowSearch(false)} className='my-auto p-2'><IoArrowBackOutline /></button>
          </div>
          <div className='w-11/12 border-2 rounded-lg'>
            <input type="search" className='border-none max-h-full h-full my-auto focus:border-none w-full focus:ring-0' placeholder='Masukkan Pencarian' value={search || ''}
            onChange={setSearch} />
          </div>
        </div>  
      </div>
