...
export default function ShareToOther({organization, users, userFilter, userOrganization}) {
    ...
    useEffect(() => {
        setFilterRole(roles.filter((role) => 
            role.name
            .toLowerCase()
            .replace(/\s+/g, '')
            .includes(queryRole?.toLowerCase().replace(/\s+/g, '')))
        )
    }, [queryRole])
            ...
            <div className='min-h-screen bg-gray-100'>
                ...
                <Container className={"sm:mt-5"}>
                    <div className='flex justify-between gap-2 sm:my-2'>
                        <div className=''>
                            <PrimaryButton className='hidden sm:block' onClick={handleOpenModalInput}>Tambah Pengguna</PrimaryButton>
                        </div>
                        ...
                        <div className='md:hidden block fixed left-2 bottom-2'>
                            {/* Mobile */}
                                <div>
                                    <button className={'btn btn-circle bg-gray-800 text-white'}  onClick={handleOpenModalInput}>
                    ...
                    </div>                    
                    {
                        organization.users.map((user, index) => 
                            <div className={`hover:bg-slate-100 ${user.pivot.is_waiting ? 'border-orange-400 border' : ''} hover:rounded-sm px-2 py-3`} key={index}>
                                {
                                    user.pivot.is_waiting ? <div className='w-full text-center text-xs my-auto bg-orange-400 text-white'>Menunggu Konfirmasi</div> : ''
                                }
                                ...
                                <div className='flex justify-between sm:px-4 grid-2 w-full'>
                                    ...
                                    {
                                        user.pivot.role !== 'admin' && <div className="dropdown dropdown-left">                                                                          
                                            <div tabIndex={0} role="button" className="bg-inherit border-none "><IoEllipsisVertical /></div>
                                            <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                                                {
                                                    userOrganization.pivot.role == 'admin' && <>
                                                        <li>
                                                            <button onClick={() => handleShowEdit(user)}>
                                                                <IoCreateOutline />Ubah Role
                                                            </button>
                                                        </li>
                                                        <li>
                                                            <button onClick={() => handleShowDelete(user)}>
                                                                <IoTrashOutline />Hapus Tautan
                                                            </button>
                                                        </li>
                                                    </>
                                                }
                                                {
                                                    ...
                                                    <li className='flex justify-between flex-row'>
                                                        <div className='bg-green-600 hover:bg-green-400 text-white'>
                                                            <Link 
                                                                href={route('organization.share-to-other.patch.confirmation', organization.id)} 
                                                                as='button' 
                                                                method='patch' 
                                                                data={{
                                                                    confirm: true, 
                                                                    user_id:userOrganization.pivot.user_id,
                                                                    organization_id:userOrganization.pivot.organization_id, 
                                                                    is_waiting: false
                                                                }}
                                                            >
                                                                Terima
            ...
            {/* Modal */}
                ...
                <Modal show={showAdd} onClose={() => setShowAdd(false)}>
                    <form 
                        ...
                        <div className="mt-6 space-y-2">
                            {
                                !isUpdate && <div className='sm:flex w-full gap-1'>
                                    ...
                                    <div className='sm:w-8/12'>
                                        ...
                                        />
                                        {
                                            errors?.user_id && <div className='text-xs text-red-500'>
                                                {errors.user_id}
                                            </div>
                                        }
                ...
                <Modal show={showDeleteConfirmation} onClose={() => setShowDeleteConfirmation(false)}>
                    <form 
                        ...
                        className="p-6">
                        <h2 className="text-lg font-medium text-gray-900">
                            Konfirmasi Hapus Penautan
                        </h2>

                        <div className="mt-6 space-y-2">
                            <div className='text-center'>
                                Apakah Anda Yakin Menghapus Tautan Dengan <span className='font-bold'>{userDelete.toUpperCase()}</span> ?
                            </div>
                            
                            <div className="mt-10 py-10 sm:mx-3 flex flex-col-reverse justify-center sm:flex-row sm:justify-end">
                                <div className='text-center'>
                                    <SecondaryButton className='w-full mt-2 sm:mt-0' onClick={() => setShowDeleteConfirmation(false)}>
                                        <div className='text-center w-full'>Batal</div>
                                    </SecondaryButton>
                                </div>
