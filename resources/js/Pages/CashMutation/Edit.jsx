...
export default function Edit({organization, newRef, date, cashAccounts, projects, programs, departments, cashMutation}) {
  ...
  const [selectedProject, setSelectedProject] = useState({
    id: cashMutation.journal.project_id, name: cashMutation.journal.project_id ? cashMutation.journal.project.name : '', code: cashMutation.journal.project_id ? cashMutation.journal.project.codes : ''
  });

  const [selectedProgram, setSelectedProgram] = useState({
    id: cashMutation.journal.program_id, name: cashMutation.journal.program_id ? cashMutation.journal.program.name : '', code: cashMutation.journal.program_id ? cashMutation.journal.program.codes : ''
  });

  const [selectedDepartment, setSelectedDepartment] = useState({
    id: cashMutation.journal.department_id, name: cashMutation.journal.department_id ? cashMutation.journal.department.name : '', code: cashMutation.journal.department_id ? cashMutation.journal.department.codes : ''
  });

  const [dateValue, setDateValue] = useState({
    startDate: cashMutation.date, 
    endDate: cashMutation.date
  });
  ...
  const {data, setData, patch, reset, errors, setError, processing} = useForm({
    ...
    'program_id': cashMutation.program_id,
    'project_id' : cashMutation.project_id,
    'department_id' : cashMutation.department_id,
    'is_approved' : cashMutation.journal.is_approved > 0 ? true : false,
    'value' : cashMutation.value,
    'accountDebit' : cashMutation.accountDebit.id,
    'accountCredit' : cashMutation.accountCredit.id,
  });
  ...
  const handleSubmit = (e) => {
    ...
    patch(route('cashflow.cash-mutation.update', {organization: organization.id, cashMutation: cashMutation.id}), {
      onSuccess: page => {
        toast.success(`Mutasi Kas Berhasil Diubah`, {
            ...
            <div className='flex flex-col sm:flex-row justify-between gap-3 mt-7'>
              <div className='sm:w-1/4 w-full text-slate-900 space-y-2'>
                ...
                <div>
                  ...
                  />
                  {
                    selectedDebitAccount?.code && <div className='absolute text-xs'>Kode: {selectedDebitAccount.code}</div>
                  }   
                </div>
              </div>
              <div className='sm:w-1/4 w-full text-slate-900 space-y-2'>
...
Edit.layout = page => <AuthenticatedLayout
  header={<Header>Ubah Mutasi Kas</Header>}
  children={page}
  user={page.props.auth.user}
  organization={page.props.organization}
  title="Ubah Mutasi Kas"
  backLink={<Link href={route('cashflow.cash-mutation',page.props.organization.id)}><IoArrowBackOutline/></Link>}
  breadcrumbs={<div className="text-sm breadcrumbs">
    ...
    <li className='font-bold'><Link href={route('cashflow',page.props.organization.id)}>Arus Kas</Link></li> 
      <li className='font-bold'><Link href={route('cashflow.cash-mutation',page.props.organization.id)}>Mutasi Kas</Link></li> 
      <li>Ubah Mutasi Kas</li>
    </ul>
  </div>}
