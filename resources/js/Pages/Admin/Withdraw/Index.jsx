...
function Index({ withdraws, searchFilter, statusFilter, affiliation, affiliators, startDate, endDate }) {
    ...
    const [selectedAffiliator, setSelectedAffiliator] = useState({ id: null, name: '', email: '' });
        ...
        <>
            ...
            <TitleMobile
                ...
                pageBefore={
                    withdraws.links[0].url ? (
                        <Link
                            href={`/admin/data-master/withdraws?page=${withdraws.current_page - 1}&search=${search}`}
                            preserveState>
                            <IoPlayBack />
                        </Link>
                    ) : (
                ...
                {withdraws.data.map((withdraw) => (
                    <div key={withdraw.id} className=' text-gray-900 py-2 px-1 border flex justify-between'>
                        <div className='space-y-3'>
                            <div>{withdraw.no_ref}</div>
                            <div className='text-sm'>
                                <div>{withdraw.user.name.toUpperCase()}</div>
                                <div>{withdraw.user.email}</div>
                            </div>
                            <div className='text-sm'>
                                <div>Diajukan Tanggal: {dayjs(withdraw.created_at).format('MMMM YYYY, DD')}</div>
                                {withdraw.status ? (
                                    <div> Dikirim Tanggal: {dayjs(withdraw.updated_at).format('MMMM YYYY, DD')} </div>
                                ) : (
                                    ''
                                )}
                            </div>
                        ...
                        <div>
                            ...
                            <div className='text-lg text-end font-bold'>IDR. {formatNumber(withdraw.value)}</div>
                        </div>
                        <div>
            ...
            <ContainerDesktop>
                <TitleDesktop>
                    ...
                    <div className='italic text-xs my-auto w-1/12 text-center'>
                        {formatNumber(withdraws.current_page * withdraws.per_page - (withdraws.per_page - 1))}-
                        {formatNumber(withdraws.to)} dari {formatNumber(withdraws.total)}
                    </div>
                    ...
                    <div className='my-auto flex space-x-2 w-1/12'>
                        <div className='my-auto'>
                            {withdraws.links[0].url ? (
                                <Link
                                    href={`/admin/withdraws?page=${withdraws.current_page - 1}&search=${search}`}
                                    preserveState>
                                    <IoPlayBack />
                                </Link>
                            ) : (
                        ...
                        <tbody>
                            {withdraws.data.map((withdraw) => (
                                <tr key={withdraw.id}>
                                    ...
                                    <td>IDR. {formatNumber(withdraw.value)}</td>
                                    <td className=''>
                                        {withdraw.status ? (
                                            <BadgeSuccess>Telah Dikirim</BadgeSuccess>
                                        ) : (
            ...
            <Modal show={showConfirmationModal} onClose={handleCloseConfirmationModal}>
                <form onSubmit={handleSubmitConfirmation}>
                    <div className='p-5'>
                        <h2 className='text-lg font-medium text-gray-900'>Konfirmasi Pengiriman Saldo Afiliasi</h2>
                        <div className='mt-6 space-y-3'>
                            ...
                            <div className='mt-6 flex sm:flex-row flex-col-reverse gap-2 sm:gap-0 sm:justify-end'>
                                <SecondaryButton onClick={handleCloseConfirmationModal}>
                                    <div className='w-full'>Batal</div>
                                </SecondaryButton>

                                {/* Mobile */}
                                <PrimaryButton className='sm:hidden' disabled={processing}>
                                    <div className='w-full'>Konfirmasi</div>
                                </PrimaryButton>
...
Index.layout = (page) => (
    <AuthenticatedLayout
        ...
        backLink={
            <Link href={route('admin.user-master')}>
                <IoArrowBackOutline />
            </Link>
        }
        breadcrumbs={
            <div className='text-sm breadcrumbs'>
                <ul>
                    <li className='font-bold'>
                        <Link href={route('admin.user-master')}>Data Master Pengguna</Link>
                    </li>
