...
export default function Index({organization, accounts, role, accountCategories, code, searchFilter, accountCategoryFilter, selectedAccountCategoryFilter}) {
    ...
    const {data, setData, reset, post, patch, delete:destroy, processing, errors, setError} = useForm({
    ...
    const getNewRef = () => {
        router.reload({ 
            ...
            onSuccess: page => {   
                setData({
                    ...data,
                    'code'  : tempData.selectedAccountCategoryId == selectedAccountCategory.id ? tempData.code : page.props.code.toString(),
    ...
    return <>
        ...
        {/* Mobile */}
            ...
            <TitleMobile 
                ...
                ? <Link href={route('data-ledger.account', {organization : organization.id, page : accounts.current_page - 1, search: search})} preserveState only={['accounts']}><IoPlayBack /></Link>
                : <div className='text-gray-300'><IoPlayBack /></div>
                }
                pageAfter={
                    accounts.links[accounts.links.length-1].url 
                    ? <Link href={`/data-ledger/${organization.id}/accounts?page=${accounts.current_page + 1}&search=${search}`}
                        only={['accounts']} preserveState>
                        <IoPlayForward />
                    </Link>
        ...
        <ContainerDesktop>
            ...
            <TitleDesktop>
                ...
                <div className='my-auto flex space-x-2 w-1/12'>
                    ...
                    <div className='my-auto'>{accounts.current_page}/{accounts.last_page}</div>
                    <div className='my-auto'>
                        {
                            accounts.links[accounts.links.length-1].url 
                            ? <Link 
                    ...
                    <div className='flex flex-col sm:flex-row w-full gap-1 mt-5'>
                        ...
                        <div className='sm:w-2/3 w-full'>
                            ...
                            />
                            {
                                ...
                                <div className='-mb-3'>
                                    <div className='text-xs text-red-500'>The account category field is requred</div>  
                                </div>
                            }
                        </div>
                    </div>
        ...
        <Modal show={showDeleteConfirmation} onClose={() => setShowDeleteConfirmation(false)}>
            ...
            >
                <h2 className="text-lg font-medium text-gray-900 text-center">
                    ...
                    {
                        data && <div>{data.name} ({data.code})</div>
                    }
                </h2>
...
Index.layout = page => <AuthenticatedLayout
    ...
    title="Data Akun"
    backLink={<Link href={route('data-ledger',page.props.organization.id)}><IoArrowBackOutline/></Link>}
    breadcrumbs={<div className="text-sm breadcrumbs">
        <ul>
            <li className='font-bold'><Link href={route('data-ledger',page.props.organization.id)}>Buku Besar</Link></li> 
            <li>Daftar Akun</li>
        </ul>
    </div>}
