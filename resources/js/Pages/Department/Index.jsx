...
export default function Index({role, organization, departments, searchFilter, code}) {
  ...
  const handleSubmitInput = (e) => {
    ...
    isEdit 
    ? patch(route('data-master.department.update', {organization: organization.id, department: selectedDepartment.id}), {
      onSuccess: () => {
    ...
    : post(route('data-master.department.post', organization.id),{
      onSuccess: () => {
  ...
  const handleSubmitDelete = (e) => {
    e.preventDefault();
    destroy(route('data-master.department.destroy', {organization: organization.id, department: selectedDepartment.id}), {
      onSuccess: () => {
    ...
    <>
      ...
      {/* Mobile */}
        ...
        <TitleMobile 
          ...
          ? <Link href={`/data-master/departments?page=${departments.current_page - 1}&search=${search}`} preserveState only={['departments']}><IoPlayBack /></Link>
          : <div className='text-gray-300'><IoPlayBack /></div>
          }
          pageAfter={
                      departments.links[departments.links.length-1].url 
                      ? <Link href={`/data-master/departments?page=${departments.current_page + 1}&search=${search}`}
                          only={['departments']} preserveState>
                          <IoPlayForward />
                      </Link>
        ...
        <ContentMobile>
        {
          ...
          <DepartmentMobile
            ...
            handleEdit={() => handleEdit(department)}
            handleShow={() => handleShow(department)}
            role={role}
          />
          )
        }
          ...
          <TitleDesktop>
            ...
            <div className='my-auto w-4/12 gap-5 justify-end'>
              ...
              <div className='w-3/12 border flex rounded-lg'>					
                ...
                <input id='search-input' name='search-input' type="search" placeholder='Cari Departemen Kegiatan' className='w-full border-none focus:outline-none focus:ring-0' value={search || ''}
                onChange={e => setSearch(e.target.value)}/>
              </div>

              <div className='italic text-xs my-auto w-1/12 text-center'>
                <PageNumber data={departments} />
              </div>
                ...
                <div className='my-auto'>
                  {
                    ...
                    ? <Link 
                          ...
                          preserveState only={['departments']}
                        >
                          <IoPlayBack />
                        </Link>
                    : <div className='text-gray-300'><IoPlayBack /></div>
                  ...
                  <div className='my-auto'>{departments.current_page}/{departments.last_page}</div>
                  <div className='my-auto'>
                      {
                          departments.links[departments.links.length-1].url 
                          ? <Link 
            ...
            <div className="mt-6 ">
              <div className='flex w-full gap-1'>
                  ...
                  <TextInput 
                    ...
                    placeholder='Nama Departemen Kegiatan'
                    value={data.name}
        ...
        <Modal show={showModalDelete} onClose={handleCloseDetailModal}>
          ...
          >
            <h2 className="text-lg font-medium text-gray-900 text-center">
              Hapus Departemen {selectedDepartment.name}
            </h2>
...
Index.layout = page => <AuthenticatedLayout
    header={<Header>Data Departemen Kegiatan</Header>}
    children={page}
    user={page.props.auth.user}
    organization={page.props.organization}
    title="Data Departemen Kegiatan"
    ...
    breadcrumbs={<div className="text-sm breadcrumbs">
        <ul>
            ...
            <li>Data Departemen Kegiatan</li>
        </ul>
    </div>}
