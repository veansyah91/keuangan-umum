...
export default function Edit({cashIn, organization, newRef, date, accounts, cashAccounts, projects, programs, departments, contacts}) {

  // state
  const [selectedAccount, setSelectedAccount] = useState(cashIn.selectedAccount);

  const [selectedContact, setSelectedContact] = useState(
    {id: cashIn.contact.id, name: cashIn.contact.name, phone:cashIn.contact.phone}
  ); 
  ...
  const [selectedProject, setSelectedProject] = useState({
    id: cashIn.journal.project_id, name: cashIn.journal.project_id ? cashIn.journal.project.name : '', code: cashIn.journal.project_id ? cashIn.journal.project.codes : ''
  });
  
  const [projectError, setProjectedError] = useState('');

  const [selectedProgram, setSelectedProgram] = useState({
    id: cashIn.journal.program_id, name: cashIn.journal.program_id ? cashIn.journal.program.name : '', code: cashIn.journal.program_id ? cashIn.journal.program.codes : ''
  });
  const [programError, setProgramedError] = useState('');

  const [selectedDepartment, setSelectedDepartment] = useState({
    id: cashIn.journal.department_id, name: cashIn.journal.department_id ? cashIn.journal.department.name : '', code: cashIn.journal.department_id ? cashIn.journal.department.codes : ''
  });

  const [departmentError, setDepartmentedError] = useState('');

  const [dateValue, setDateValue] = useState({
    startDate: cashIn.date, 
    endDate: cashIn.date
  });
  ...
  const {data, setData, patch, reset, errors, setError, processing} = useForm({
    ...
    'program_id': cashIn.journal.program_id,
    'project_id' : cashIn.journal.project_id,
    'department_id' : cashIn.journal.department_id,
    'is_approved' : cashIn.journal.is_approved,
    'cash_account_id' : cashIn.cashAccount.id,
    'accounts' : cashIn.accounts,
  });
  ...
  useEffect(() => {
    if(prevDate!==undefined) {
      ...
      let oldDateFormatted = dayjs(cashIn.date);
  ...
  const handleSubmit = (e) => {
    ...
    patch(route('cashflow.cash-in.update', {organization: organization.id, cashIn: cashIn.id}), {
      onSuccess: page => {
        toast.success(`Kas Masuk Berhasil Diubah`, {
  ...
  const handleSelectedCashAccount = (selected) => {
    setSelectedCashAccount(
      {id: selected.id, name: selected.name, code: selected.code, is_cash: true}
    );
  ...
  const handleSelectedAccount = (selected, index) => {
    ...
    tempData[index] = {...tempData[index],
      id: selected.id, name: selected.name, code: selected.code, is_cash:selected.is_cash
    }
  ...
  const handleAddAccount = () => {
    const tempDetail = [...data.accounts,
      {id: null, name: '', code: '', value:0, is_cash: false}
    ];
            ...
            <div className='flex flex-col sm:flex-row justify-between gap-3'>
              ...
              <div className='sm:w-1/4 w-full text-slate-900 space-y-2'>
                ...
                <div>
                  ...
                  />
                  {
                    selectedCashAccount?.code && <div className='absolute text-xs'>Kode: {selectedCashAccount.code}</div>
                  }   
                </div>
              </div>
            ...
            <div className='mt-5'>
              <div className='w-full'>
                ...
                </div>
                {
                  ...
                  <div className='md:flex gap-3 py-3 mt-2 space-y-2 border md:border-none rounded-lg border-slate-900 px-3' key={index}>
                    ...
                    <div className='md:w-5/12 w-full relative'>
                      ...
                      />
                      {
                        selectedAccount[index]?.code && <div className='absolute text-xs'>Kode: {selectedAccount[index].code}</div>
                      }                        
                    </div>
                    ...
                    <div className='md:w-1/12 w-full text-center my-auto'>
                      <button 
                      type='button' className='md:text-red-500 text-white bg-red-500 md:bg-white text-xl hover:bg-slate-200 rounded-full p-2 w-full'
                ...
                <div className='py-3 mt-4'>
                  <button className='w-full border bg-slate-100 hover:bg-slate-200 text-slate-500 py-3 flex justify-center gap-2' type='button' onClick={handleAddAccount}>
                    <div className='my-auto'><IoAddOutline /></div>
...
Edit.layout = page => <AuthenticatedLayout
  header={<Header>Ubah Kas Masuk</Header>}
  children={page}
  ...
  breadcrumbs={<div className="text-sm breadcrumbs">
    <ul>
    <li className='font-bold'><Link href={route('cashflow',page.props.organization.id)}>Arus Kas</Link></li> 
      <li className='font-bold'><Link href={route('cashflow.cash-in',page.props.organization.id)}>Kas Masuk</Link></li> 
      <li>Ubah Kas Masuk</li>
    </ul>
  </div>}
