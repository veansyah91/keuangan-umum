...
import CashInMobile from './Components/CashinMobile';
import CashInDesktop from './Components/CashinDesktop';
...
export default function Index({ cashIns, startDate, endDate, role, organization, searchFilter, isApproved, projects, programs, departments }) {
  ...
  const handleDelete = (cashIn) => {
    setModalDelete({
			title: `Hapus Data Kas Masuk No Ref ${cashIn.no_ref}`
		});
		setData({
			id: cashIn.id,
  ...
  const handleSubmitDelete = (e) => {
		...
		destroy(route('cashflow.cash-in.delete', {organization: organization.id, cashIn: data.id}), {
			onSuccess: () => {
				reset();
				toast.success(`Kas Masuk Berhasil Dihapus`, {
  ...
  return (
    <>
      ...
      {
      (role !== 'viewer') && <Link href={route('cashflow.cash-in.create', organization.id)}><AddButtonMobile label={"Tambah"}/></Link> 
      }
      <TitleMobile 
        zIndex={'z-50'}
        search={search}
        ...
        pageBefore={
              cashIns.links[0].url 
        ? <Link href={`/cashflows/${organization.id}/cash-in?page=${cashIns.current_page - 1}&search=${search}`} preserveState only={['cashIns']}><IoPlayBack /></Link>
        : <div className='text-gray-300'><IoPlayBack /></div>
        }
        pageAfter={
              cashIns.links[cashIns.links.length-1].url 
              ? <Link href={`/cashflows/${organization.id}/cash-in?page=${cashIns.current_page + 1}&search=${search}`}
                only={['cashIns']} preserveState>
                <IoPlayForward />
              </Link>
        ...
        page={
          <>
            {cashIns.current_page}/{cashIns.last_page}
          </>
        }
        data={cashIns}
        hasFilter={true}
      ...
      <ContentMobile>
      {
        cashIns.data.map(cashIn => 
          <CashInMobile
            cashIn={cashIn}
            key={cashIn.id}
            handleDelete={() => handleDelete(cashIn)}
            role={role}
          />
        )
      }
      ...
      <ContainerDesktop>
        <TitleDesktop>
          <div className='my-auto w-5/12'>
            {
              (role !== 'viewer') &&
              <Link href={route('cashflow.cash-in.create', organization.id)}>
          ...
          <div className='w-3/12 border flex rounded-lg'>					
            ...
            <input id='search-input' name='search-input' type="search" placeholder='Cari Kas Masuk' className='w-full border-none focus:outline-none focus:ring-0' value={search || ''}
            onChange={e => setSearch(e.target.value)}/>
          </div>

          <div className='italic text-xs my-auto w-1/12 text-center'>
              <PageNumber data={cashIns} />
          </div>
          ...
          <div className='my-auto flex space-x-2 w-1/12'>
            <div className='my-auto'>
              {
                cashIns.links[0].url 
                ? <Link 
                    href={`/cashflows/${organization.id}/cash-in?page=${cashIns.current_page - 1}&search=${search}`}
                    preserveState only={['cashIns']}
                  >
                    <IoPlayBack />
                  </Link>
                : <div className='text-gray-300'><IoPlayBack /></div>
            ...
            <div className='my-auto'>{cashIns.current_page}/{cashIns.last_page}</div>
            <div className='my-auto'>
              {
                cashIns.links[cashIns.links.length-1].url 
                ? <Link 
...
Index.layout = page => <AuthenticatedLayout
    header={<Header>Kas Masuk</Header>}
    children={page}
    user={page.props.auth.user}
    organization={page.props.organization}
    title="Kas Masuk"
    backLink={<Link href={route('cashflow',page.props.organization.id)}><IoArrowBackOutline/></Link>}
