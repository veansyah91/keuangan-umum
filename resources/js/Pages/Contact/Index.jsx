  ...
  return (
    <>
      {/* Mobile */}
        ...
        {
          (role !== 'viewer') && <Link href={route('data-master.contact.create', organization.id)}><AddButtonMobile label={"Tambah"}/></Link>
        }
        <TitleMobile 
            zIndex={'z-50'}
            search={search}
            ...
            ? <Link href={`/data-ledger/${organization.id}/contacts?page=${contacts.current_page - 1}&search=${search}`} preserveState only={['contacts']}><IoPlayBack /></Link>
            : <div className='text-gray-300'><IoPlayBack /></div>
            }
            pageAfter={
                contacts.links[contacts.links.length-1].url 
                ? <Link href={`/data-ledger/${organization.id}/contacts?page=${contacts.current_page + 1}&search=${search}`}
                    only={['contacts']} preserveState>
                    <IoPlayForward />
                </Link>
      ...
      <ContainerDesktop>  
        <TitleDesktop>
          ...
          <div className='w-3/12 border flex rounded-lg'>
            ...
            <input id='search-input' name='search-input' type="search" placeholder='Cari Kontak' className='w-full border-none focus:outline-none focus:ring-0' value={search || ''}
          ...
          <div className='my-auto flex space-x-2 w-1/12'>
            ...
            <div className='my-auto'>{contacts.current_page}/{contacts.last_page}</div>
            <div className='my-auto'>
                {
                    contacts.links[contacts.links.length-1].url 
                    ? <Link 
