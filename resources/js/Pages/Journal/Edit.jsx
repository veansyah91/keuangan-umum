...
export default function Edit({organization, accounts, journal, ledgers, program, department, project, programs, departments, projects}) {
  ...
  useEffect(() => {
    ...
    ledgers.map(ledger => {
      tempSelectAccount = [...tempSelectAccount, {
        id: ledger.account.id, name: ledger.account.name, code: ledger.account.code, is_cash:ledger.account.is_cash},
      ];
      tempAccountError = [...tempAccountError, ''];
      tempAccounts = [...tempAccounts, 
        {id: ledger.account.id, name: ledger.account.name, code: ledger.account.code, debit: ledger.debit, credit:ledger.credit, is_cash:ledger.account.is_cash}]
    });
            ...
            <div className='sm:mx-auto px-3 sm:px-5'>
              ...
              <div className='md:flex font-bold gap-3'>
                ...
                <div className='md:w-7/12 flex border-t py-3 gap-12 px-3'>
                  ...
                  <div className={`w-1/2 text-end ${Math.abs(debit-credit) > 0 && 'text-red-500'}`}>IDR {formatNumber(Math.abs(debit-credit))}</div>
                </div>              
              </div>
              <div className='md:flex font-bold gap-3'>
                <div className='w-5/12'></div>
...
Edit.layout = page => <AuthenticatedLayout
  ...
  backLink={<Link href={route('data-ledger.journal',page.props.organization.id)}><IoArrowBackOutline/></Link>}
  breadcrumbs={<div className="text-sm breadcrumbs">
    <ul>
      ...
      <li className='font-bold'><Link href={route('data-ledger.journal',page.props.organization.id)}>Jurnal Umum</Link></li> 
