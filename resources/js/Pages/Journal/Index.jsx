...
export default function Index({startDate, endDate, journals, role, organization, searchFilter, isApproved, projects, programs, departments}) {
  ...
  <>
    {/* Mobile */}
		...
		{
		(role !== 'viewer') && <Link href={route('data-ledger.journal.create', organization.id)}><AddButtonMobile label={"Tambah"}/></Link> 
		}
		<TitleMobile 
			zIndex={'z-50'}
			search={search}
			...
			? <Link href={`/data-ledger/journals?page=${journals.current_page - 1}&search=${search}`} preserveState only={['journals']}><IoPlayBack /></Link>
			: <div className='text-gray-300'><IoPlayBack /></div>
			}
			pageAfter={
						journals.links[journals.links.length-1].url 
						? <Link href={`/data-ledger/journals?page=${journals.current_page + 1}&search=${search}`}
							only={['journals']} preserveState>
							<IoPlayForward />
						</Link>
		...
		<ContainerDesktop>
			<TitleDesktop>
				...
				<div className='w-3/12 border flex rounded-lg'>					
					...
					<input id='search-input' name='search-input' type="search" placeholder='Cari Jurnal' className='w-full border-none focus:outline-none focus:ring-0' value={search || ''}
				...
				<div className='my-auto flex space-x-2 w-1/12'>
					...
					<div className='my-auto'>{journals.current_page}/{journals.last_page}</div>
					<div className='my-auto'>
						{
							journals.links[journals.links.length-1].url 
							? <Link 
