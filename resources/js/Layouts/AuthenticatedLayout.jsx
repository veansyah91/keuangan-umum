										...
										<div className='shrink-0 flex items-center'>
											...
											{route().current('admin*') ? (
												<div className='hidden space-x-8 md:-my-px md:ms-10 md:flex'>
													...
													<NavLink
														href={route('admin.data-master')}
														active={route().current('admin.data-master*')}>
														DATA MASTER
													</NavLink>
													<NavLink
														href={route('admin.user-master')}
														active={route().current('admin.user-master*')}>
														DATA PENGGUNA
													</NavLink>
													...
													<NavLink
														href={route('admin.organization.menu')}
												...
												<div className='hidden space-x-8 md:-my-px md:ms-10 md:flex'>
													...
													<NavLink
														href={route('data-master', organization?.id)}
														active={route().current('data-master*')}>
														DATA MASTER
													</NavLink>
													<NavLink
														href={route('data-ledger', organization?.id)}
														active={route().current('data-ledger*')}>
														BUKU BESAR
													</NavLink>
													<NavLink
														href={route('cashflow', organization?.id)}
														active={route().current('cashflow*')}>
														ARUS KAS
													</NavLink>
													<NavLink
														href={route('report', organization?.id)}
										...
										<div className='ms-3 relative'>
											<Dropdown>
												...
												<Dropdown.Content>
													{!route().current('admin*') && (
														<>
															<Dropdown.Link href={route('organization')}>
																Daftar Organisasi
															</Dropdown.Link>
															<hr />
														</>
					...
					<div className='drawer-side z-50'>
						...
						{route().current('admin*') ? (
							...
							<ul className='menu p-4 w-64 min-h-full text-[#4f8315] bg-white font-lg bg-opacity-95'>
								...
								<li
									className={`${route().current('admin.dashboard*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
									<Link href={route('admin.dashboard')}>Dasbor</Link>
								</li>
								<li
									className={`${route().current('admin.data-master*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
									<Link href={route('admin.data-master')}>Data Master</Link>
								</li>
								<li
									className={`${route().current('admin.user-master*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
									<Link href={route('admin.user-master')}>Data Pengguna</Link>
								</li>
								<li
						...
						<ul className='menu p-4 w-64 min-h-full text-[#4f8315] bg-white font-lg bg-opacity-95'>
							{/* Sidebar content here */}
							<li
								className={`${route().current('dashboard*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
								<Link href={route('dashboard', organization.id)}>Dasbor</Link>
							</li>
							<li
								className={`${route().current('data-master*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
								<Link href={route('data-master', organization.id)}>Data Master</Link>
							</li>
							<li
								className={`${route().current('data-ledger*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
								<Link href={route('data-ledger', organization.id)}>Buku Besar</Link>
							</li>
							<li
								className={`${route().current('cashflow*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
								<Link href={route('cashflow', organization.id)}>Arus Kas</Link>
							</li>
							<li
								className={`${route().current('report*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
								<Link href={route('report', organization.id)}>Laporan</Link>
							</li>
							...
							{role === 'admin' && (
								<>
									...
									<li
										className={`${route().current('menu-setting*') ? 'border-l-2 border-[#4f8315] font-bold' : ''}`}>
